...
map(mtcars, mean, na.rm = T) # will return a list
map_df(mtcars, mean, na.rm = T) # will return a df
...
# it sequences through the first list, and pastes it with the second list
# also use expand.grid(), which creates factorial combinations of everything
...
z # what happens inside the function stays inside the function
```
...
z <- x + y + a # but function can see the variable outside --> but this will cause problems
...
map(mtcars, ~ sd(.) / mean(.)) # . is the placeholder for the data that's being passed
...
Way to get a non-dataframe output::: Why would you do this with `select()`? this is a way to not get a data frame back - just get the values back. 
...
For those of you who prefer a more interactive approach, you can also use the `jsonedit` function in the `listviewer` package - awesome
...
got_chars[1:15] %>% # with just single brackets it keeps it as a list
  map_chr('name')
...
# but this would work says Juan
# got_chars[1:5] %>%
#   ?invoke(c('name','allegiances')) %>%
#   listviewer::jsonedit()
...
map(`[`, c('name', 'allegiances')) %>% # [ = turn my list operation into a function
listviewer::jsonedit()
```
...
map_chr(thing, c('z','w')) # z, w is taking the place of a function; a shortcut for .$
...
This is where a lot of the power of `purrr` starts to come in, allowing you to easily apply functions across nested layers of a list as you poke through them.
...
got_chars %>%
  set_names(map_chr(.,'name')) %>%
  map(`[`,c('name','allegiances')) %>%
  keep(~str_detect(.$name, 'Lannister')) %>% # equivalent of dplyr::filter. keep() needs an anonmymous funciton `~`
  listviewer::jsonedit()
```
...
Hmmm, that doesn't look good, what's up with Will? What happens if I try and use `keep` (list `filter`) here? -> a lot of errors are when there are data missing from the list

`tidyr::nest()` and `tidyr::unnest()` is kind of like unlist... also see purr::collapse.
...
## shorter than below but...
got_list <- got_chars %>%
  map(`[`, c('name','aliases','allegiances','born'))

## does the same as above but is way more explicit about the output type w/ map_chr
got_list <-  got_chars %>% { ## notice the brackets again like we saw above
  list(
    name = map_chr(.,'name'),
    aliases = map(.,'aliases'),
...
## example above could have been done more with tibble and mutate; this isn't the best way to do it but it's the way to do it with purrr
...
Tracey: would you ever want to leave things in a list? Tyler: think of df::list as spreadsheet::database. Sometimes you want it 

Ben Best: Spatial is a great example of how to interact with lists. This is increasingly more required for online databases. Each time you use an html widget it's pushing your json object, and when you read it in and out as a heirarchical list. Also xml

Does purrr have built-in API interaction?

purrr is the missing link between being tidy but not using dfs. tidyverse is a glorified pivot table
...
# Store these models as a tibble
...
model_frame <- model_frame %>%
  ...
  aic = map_dbl(fit, ~ AIC(.))) # why do you need the ~ here? when in doubt, ~XXX(.)
...
mutate(root_mean_sq_error = map2_dbl(fit, test, modelr::rmse))
...
instead of looping over, you're sweeping across. 
...
safe_log <-  safely(log) # define a functiony thing
...
purrr::walk(plot_files, plot_foo) # walk gives but doesn't output
